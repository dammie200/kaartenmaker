...
def build_selection(data: dict) -> Selection:
    try:
...
def layer_selection(data: dict) -> Dict[str, LayerSelection]:
...
def plot_layers(ax, selection: Selection, layers: Dict[str, LayerSelection]):
...
def generate_map(data: dict):
    ...
    layers = layer_selection(data)
    fmt = (data.get("format", "png") or "png").lower()
    ...
    plot_layers(ax, selection, layers)
    ...
    return buffer, mimetypes.get(fmt, "application/octet-stream"), fmt
