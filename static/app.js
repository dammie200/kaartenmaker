const TAGS_PER_LAYER = {
  'Gebouwen': { building: true },
  'Hoofdwegen': { highway: ['motorway', 'primary', 'secondary', 'tertiary', 'trunk', 'residential'] },
...
const layerListRoot = document.getElementById('layer-list');
...
function renderShapeParams(shape) {
  ...
  if (shape === 'circle') {
    ...
    shapeParamsRoot.innerHTML = `
      ...
      <select id="paper">
        ${Object.keys(PAPER_FORMATS).map(key => `<option value="${key}">${key}</option>`).join('')}
      </select>
...
function buildLayerControls() {
  layerListRoot.innerHTML = '';
  Object.entries(TAGS_PER_LAYER).forEach(([name, tags]) => {
    ...
    const hasSublayers = Object.values(tags).some(value => Array.isArray(value));
    if (hasSublayers) {
      Object.entries(tags).forEach(([key, values]) => {
        if (!Array.isArray(values)) return;
        const sub = document.createElement('div');
        sub.className = 'sublayer';
    ...
    layerListRoot.appendChild(details);
  });
}
...
function updateSelection() {
  ...
  if (shape === 'circle') {
    ...
    const [wMm, hMm] = PAPER_FORMATS[paper];
